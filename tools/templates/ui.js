...
const TARGETING_3D_MODELS={
  ...
  "Trooper":{slug:"trooper",cameraVector:[0,.04,-1.05]},
  "Commissar":{slug:"commissar",cameraVector:[0,.04,-1.05]},
  ...
  "Agitator":{slug:"agitator",cameraVector:[0,.04,-1.05]},
  "Radical":{slug:"radical",cameraVector:[0,.04,-1.05]},
...
function targeting3dPointerHit(clientX,clientY,useCenter=false){
  if(!targeting3d.camera||!targeting3d.plane)return null;const host=$('targeting3dCanvas'),rect=host.getBoundingClientRect(),{THREE}=globalThis.HD2Three,pointer=new THREE.Vector2(useCenter?0:(clientX-rect.left)/rect.width*2-1,useCenter?0:-(clientY-rect.top)/rect.height*2+1),raycaster=new THREE.Raycaster();raycaster.setFromCamera(pointer,targeting3d.camera);const targets=[...targeting3d.hulls.filter(hull=>!hull.userData.hd2GeometryOnly),targeting3d.plane],visibility=targets.map(target=>target.visible),gridVisible=targeting3d.gridGroup?.visible;targets.forEach(target=>target.visible=true);if(targeting3d.gridGroup)targeting3d.gridGroup.visible=true;const hit=raycaster.intersectObjects(targets,false)[0]||null;targets.forEach((target,index)=>target.visible=visibility[index]);if(targeting3d.gridGroup)targeting3d.gridGroup.visible=gridVisible;if(!hit)return null;const direct=hit.object===targeting3d.plane?null:hit.object,point=v3(hit.point.x,hit.point.y,hit.point.z),groundDistance=Math.hypot(point.x-targeting3d.gridCenter.x,point.z-targeting3d.gridCenter.z),hullDistance=nearestTargetingHullDistance(point);return {mesh:direct,point,groundDistance,hullDistance,directPoolKey:direct?.userData.hd2PoolKey||null,angleIndex:direct?targeting3dImpactAngleIndex(hit,raycaster.ray.direction):0,label:direct?targeting3dZoneLabel(direct.userData.hd2Damage):'Ground impact'};
}
...
function updateTargeting3dAim(hit,clientX,clientY){
  ...
  const profile=activeTargeting3dProfile(),blast=profile?.outer?` · blast ${fmt(profile.inner)}–${fmt(profile.outer)} m`:'';$('targeting3dGroundDistance').textContent=`${fmt(hit.groundDistance)} m`;$('targeting3dHullDistance').textContent=Number.isFinite(hit.hullDistance)?`${fmt(hit.hullDistance)} m`:'—';$('targeting3dAimReadout').textContent=`${hit.mesh?'Direct hit':'Ground impact'} · ${hit.label} · ${fmt(hit.groundDistance)} m ground offset${blast}`;updateTargetingRingHighlight(hit.groundDistance);
}
...
function renderTargeting3dImpactVisuals(){
  ...
  visibleImpacts.forEach((impact,index)=>{const marker=new THREE.Mesh(new THREE.SphereGeometry(.16,14,8),new THREE.MeshBasicMaterial({color:impact.directPoolKey?0xff684d:0xffe10a,depthTest:false}));marker.position.set(impact.position.x,impact.position.y,impact.position.z);marker.renderOrder=12;group.add(marker);if(barrage)group.add(makeTargetingLabel(impact.id||String(index+1),impact.position.x,impact.position.z));const showRadii=visibleImpacts.length<=3||index===visibleImpacts.length-1;if(showRadii){for(const [radius,color,opacity] of [[profile.delivery.shockwave,0xb9d3e2,.1],[profile.outer,0x5da9d6,.18],[profile.inner,0xffe10a,.28]]){if(!radius)continue;const sphere=new THREE.Mesh(new THREE.SphereGeometry(radius,24,12),new THREE.MeshBasicMaterial({color,wireframe:true,transparent:true,opacity,depthWrite:false}));sphere.position.copy(marker.position);sphere.renderOrder=5;group.add(sphere);}}});targeting3d.scene.add(group);targeting3d.impactGroup=group;
}
...
function renderTargeting3dResult(){
  const policy=$('targeting3dPolicy').value,result=targeting3d.results?.[policy],summaryHost=$('targeting3dSplashSummary'),zonesHost=$('targeting3dSplashZones'),eventsHost=$('targeting3dSplashEvents');if(!result){$('targeting3dSplashTitle').textContent='Place an impact to simulate damage';summaryHost.replaceChildren();zonesHost.replaceChildren();eventsHost.replaceChildren();$('targeting3dSplashRange').textContent='';return;}
...
function placeTargeting3dImpact(hit=targeting3d.hover){
  if(!hit||!targeting3d.profile)return;const delivery=$('targeting3dDelivery').value,profile=activeTargeting3dProfile(),{THREE}=globalThis.HD2Three;if(delivery==='generated'&&/barrage/.test(profile.delivery.kind)){const beacon=v3(hit.point.x,0,hit.point.z);targeting3d.beacon=beacon;const pattern=generateBarragePattern(profile,beacon,THREE.MathUtils.degToRad(+$('targeting3dHeading').value||0),Math.max(1,+$('targeting3dSeed').value||1),{moreGuns:$('targeting3dMoreGuns').checked,atmosphericMonitoring:$('targeting3dAtmospheric').checked,highDensityExplosives:$('targeting3dHighDensity').checked});targeting3d.impacts=pattern.map(resolveVerticalBarrageImpact);}else{const impact={position:{...hit.point},directPoolKey:hit.directPoolKey,angleIndex:hit.angleIndex||0,time:targeting3d.impacts.length,seed:Math.max(1,+$('targeting3dSeed').value||1)};targeting3d.beacon=null;targeting3d.impacts=delivery==='manual'?[...targeting3d.impacts,impact]:[impact];}runTargeting3dSimulation();$('targeting3dStatus').textContent=`${targeting3d.impacts.length} impact${targeting3d.impacts.length===1?'':'s'} simulated`;
}

function updateTargeting3dWeapon(){
  targeting3d.profile=selectedTargeting3dProfile();const barrage=/barrage/.test(targeting3d.profile?.delivery.kind||''),delivery=$('targeting3dDelivery'),current=delivery.value;[...delivery.options].forEach(option=>option.hidden=!barrage&&option.value!=='single');delivery.value=barrage?(current==='single'?'generated':current):'single';$('targeting3dSeedField').hidden=!barrage;$('targeting3dHeadingField').hidden=targeting3d.profile?.delivery.kind!=='walking-barrage';$('targeting3dUpgrades').hidden=!barrage;$('estimateTargeting3dBarrage').hidden=!barrage||delivery.value==='manual';targeting3d.impacts=[];targeting3d.beacon=null;targeting3d.results=null;if(targeting3d.initialized)createTargeting3dGrid();runTargeting3dSimulation();
}

function updateTargeting3dDelivery(){
  const barrage=/barrage/.test(targeting3d.profile?.delivery.kind||''),delivery=$('targeting3dDelivery').value;$('estimateTargeting3dBarrage').hidden=!barrage||delivery==='manual';targeting3d.impacts=[];targeting3d.beacon=null;targeting3d.results=null;createTargeting3dGrid();runTargeting3dSimulation();
}

function regenerateTargeting3dBarrage(){
  createTargeting3dGrid();if(targeting3d.beacon&&$('targeting3dDelivery').value==='generated')placeTargeting3dImpact({point:{...targeting3d.beacon},directPoolKey:null});else runTargeting3dSimulation();
}
...
function setTargeting3dMode(mode){
  ...
  if(targeting3d.controls&&globalThis.HD2Three){const {THREE}=globalThis.HD2Three;targeting3d.controls.mouseButtons.LEFT=simulate?null:THREE.MOUSE.ROTATE;targeting3d.controls.mouseButtons.RIGHT=THREE.MOUSE.ROTATE;targeting3d.controls.mouseButtons.MIDDLE=THREE.MOUSE.DOLLY;targeting3d.controls.touches.ONE=simulate?null:THREE.TOUCH.ROTATE;targeting3d.controls.touches.TWO=simulate?(THREE.TOUCH.DOLLY_ROTATE??THREE.TOUCH.DOLLY_PAN):THREE.TOUCH.DOLLY_PAN;}
  if(simulate){fillTargeting3dWeapons();updateTargeting3dWeapon();$('targeting3dSubtitle').textContent=hasCoarsePointer?'Use two fingers to orbit or pinch, then place the fixed-reticle impact.':'Mouse: aim normally, right-drag to orbit, wheel to zoom. Trackpad: one finger aims, two-finger drag orbits, and pinch zooms.';if(hasCoarsePointer)updateTargeting3dAim(targeting3dPointerHit(0,0,true));}else{$('targeting3dSubtitle').textContent='Click a highlighted hull to inspect its game-derived damage zone.';targeting3d.hulls.forEach(hull=>{if(hull!==targeting3d.selected)hull.material=hull.userData.hd2NormalMaterial;});}
}
...
function selectTargeting3dHull(mesh){
  ...
  mesh.material=mesh.userData.hd2SelectedMaterial;const data=mesh.userData.hd2Collision||{},damage=mesh.userData.hd2Damage,bone=data.boneName||data.parentNodeName||`Bone 0x${data.boneHash||'--------'}`,mountLabel=data.mountLabel,number=Number(data.recordIndex)+1;$('targeting3dCollider').textContent=data.bodyProxy?`${data.proxyLabel||'Damage-zone'} proxy`:mountLabel?`${mountLabel} · hitbox ${number}`:`Hull ${number} · 0x${data.colliderHash}`;
  if(damage){const hp=damage.mountedMainHealth?`${damage.mountedMainHealth} HP`:damage.health===-1?'Main HP':`${damage.health} HP`,durability=`${Math.round((damage.projectileDurableResistance??0)*100)}% durability`,zone=targeting3dZoneLabel(damage),analog=data.bodyProxy?' · inferred body proxy':data.proxyMode?' · Gatekeeper / War Strider analog':'',layers=damage.zoneStack||[];
    ...
    else{$('targeting3dBone').textContent=`${zone} · ${hp} · AV${damage.armor} · ${durability}`;$('targeting3dEvidence').textContent=`${damage.evidenceLabel||'HealthComponent actor match'}${analog} · ${bone}`;$('targeting3dStatus').textContent=data.bodyProxy?`Selected inferred ${zone} proxy`:mountLabel?`Selected ${zone} · ${mountLabel} hitbox ${number}`:`Selected ${zone} · hull ${number} of ${targeting3d.hulls.length}`;}}
...
function targeting3dWheelLooksLikeTrackpad(event){
  if(event.ctrlKey||event.metaKey||event.deltaMode!==WheelEvent.DOM_DELTA_PIXEL)return false;
  const x=Math.abs(event.deltaX),y=Math.abs(event.deltaY);
  return x>.01||(y>0&&(y<80||Math.abs(y%100)>.01));
}
...
function bindTargeting3dRaycast(){
  ...
  host.addEventListener('wheel',event=>{if(!targeting3dWheelLooksLikeTrackpad(event))return;event.preventDefault();event.stopImmediatePropagation();orbitTargeting3dFromTrackpad(event);},{capture:true,passive:false});
  ...
  host.addEventListener('keydown',event=>{if(targeting3d.mode!=='simulate')return;if(event.key==='Enter'){event.preventDefault();const hit=centerHit();updateTargeting3dAim(hit);placeTargeting3dImpact(hit);return;}if(event.key==='Escape'){event.preventDefault();updateTargeting3dAim(null);return;}const delta={ArrowLeft:[-.25,0],ArrowRight:[.25,0],ArrowUp:[0,-.25],ArrowDown:[0,.25]}[event.key];if(!delta||!targeting3d.impacts.length)return;event.preventDefault();targeting3d.impacts=targeting3d.impacts.map(impact=>({...impact,position:{...impact.position,x:impact.position.x+delta[0],z:impact.position.z+delta[1]},directPoolKey:null}));if(targeting3d.beacon)targeting3d.beacon={...targeting3d.beacon,x:targeting3d.beacon.x+delta[0],z:targeting3d.beacon.z+delta[1]};runTargeting3dSimulation();});
}
