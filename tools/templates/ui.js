...
function setTargeting3dMode(mode){
  ...
  if(targeting3d.controls&&globalThis.HD2Three){const {THREE}=globalThis.HD2Three;targeting3d.controls.enableRotate=true;targeting3d.controls.enablePan=true;targeting3d.controls.screenSpacePanning=false;targeting3d.controls.mouseButtons.LEFT=THREE.MOUSE.ROTATE;targeting3d.controls.mouseButtons.RIGHT=THREE.MOUSE.ROTATE;targeting3d.controls.mouseButtons.MIDDLE=THREE.MOUSE.DOLLY;targeting3d.controls.touches.ONE=simulate?THREE.TOUCH.PAN:THREE.TOUCH.ROTATE;targeting3d.controls.touches.TWO=simulate?(THREE.TOUCH.DOLLY_ROTATE??THREE.TOUCH.DOLLY_PAN):THREE.TOUCH.DOLLY_PAN;}
  if(simulate){fillTargeting3dWeapons();updateTargeting3dWeapon();$('targeting3dSubtitle').textContent=hasCoarsePointer?'Drag with one finger to pan the ground under the fixed reticle. Use two fingers to orbit or pinch.':'Click to place; left-drag or right-drag to orbit. Trackpad: one finger aims, two-finger drag orbits, and pinch zooms.';if(hasCoarsePointer)updateTargeting3dAim(targeting3dPointerHit(0,0,true));}else{$('targeting3dSubtitle').textContent='Click a highlighted hull to inspect it; drag or right-drag to orbit.';targeting3d.hulls.forEach(hull=>{if(hull!==targeting3d.selected)hull.material=hull.userData.hd2NormalMaterial;});}
}
