...
const TARGETING_3D_MODELS={
  ...
  "Annihilator Tank":{slug:"annihilator-tank",assetSlug:"automaton-tank-base",damageSlug:"automaton-tank-base",collisionGlb:"assets/models/automaton-tank-base-collision-runtime.glb",renderGlb:"assets/models/automaton-tank-base-authentic-render.glb",cameraVector:[-.68,.26,-1.45],mountManifest:"assets/models/annihilator-tank-mounted-units.manifest.json"},
  "Shredder Tank":{slug:"shredder-tank",assetSlug:"automaton-tank-base",damageSlug:"automaton-tank-base",collisionGlb:"assets/models/automaton-tank-base-collision-runtime.glb",renderGlb:"assets/models/automaton-tank-base-authentic-render.glb",cameraVector:[-.68,.26,-1.45],mountManifest:"assets/models/shredder-tank-mounted-units.manifest.json"},
  "Barrager Tank":{slug:"barrager-tank",assetSlug:"automaton-tank-base",damageSlug:"automaton-tank-base",collisionGlb:"assets/models/automaton-tank-base-collision-runtime.glb",renderGlb:"assets/models/automaton-tank-base-authentic-render.glb",cameraVector:[-.68,.26,-1.45],mountManifest:"assets/models/barrager-tank-mounted-units.manifest.json"},
  "Cannon Turret":{slug:"cannon-turret",assetSlug:"automaton-heavy-cannon-turret",damageSlug:"automaton-heavy-cannon-turret",collisionGlb:"assets/models/automaton-heavy-cannon-turret-collision-runtime.glb",renderGlb:"assets/models/automaton-heavy-cannon-turret-authentic-render.glb",cameraVector:[-.62,.22,-1.45]},
...
function fillWeapons(select){const nodes=[];for(const category of ["Primary","Secondary","Support","Throwable","Vehicle","Emplacement","Stratagem"]){const classes={};WEAPONS.forEach((weapon,index)=>{if(weapon.cat===category)(classes[weaponClass(weapon)]=classes[weaponClass(weapon)]||[]).push([weapon,index]);});for(const cls of Object.keys(classes).sort()){const group=document.createElement("optgroup");group.label=category+" · "+cls;classes[cls].sort((a,b)=>a[0].name.localeCompare(b[0].name)).forEach(([weapon,index])=>{const option=new Option(weapon.name,index);option.dataset.filter=category;group.append(option);});nodes.push(group);}}select.replaceChildren(...nodes);}
...
function activeTargeting3dProfile(){
  const profile=targeting3d.profile;if(!profile)return null;const scale=/barrage/.test(profile.delivery?.kind||'')&&$('targeting3dHighDensity')?.checked?1.1:1;
...
function targetingGridRadius(){
  if(!targeting3d.modelBounds||!targeting3d.gridCenter)return 20;const size=targeting3d.modelBounds.getSize(new globalThis.HD2Three.THREE.Vector3()),footprint=Math.hypot(size.x,size.z)/2,profile=targeting3d.profile,delivery=profile?.delivery||{},blast=profile?.outer||12,pattern=delivery.kind==='walking-barrage'?(delivery.salvoStep||25)*((delivery.salvos||5)+1)+(delivery.spread||25)+blast:delivery.kind==='eagle-strafing'?(delivery.length||50)+blast:delivery.kind==='eagle-airstrike'?((delivery.count||6)-1)*(delivery.spacing||8)/2+blast:delivery.searchRadius?delivery.searchRadius+blast:delivery.spread?delivery.spread+blast:0;return Math.ceil(Math.max(15,footprint+8,blast+6,pattern));
}
...
function makeGroundClippedBlastSphere(radius,color,opacity,position){
  // Three.js discards the negative side of a local clipping plane. Point the
  // normal upward so only fragments below the targeting ground are removed.
...
function placeTargeting3dImpact(hit=targeting3d.hover){
  if(!hit||!targeting3d.profile)return;const delivery=$('targeting3dDelivery').value,profile=activeTargeting3dProfile(),{THREE}=globalThis.HD2Three;if(profile.delivery.kind==='guided-top-attack'){targeting3d.designation={...hit,point:{...hit.point}};targeting3d.beacon=null;targeting3d.impacts=[resolveGuidedTopAttackImpact(targeting3d.designation,profile)];}else if(delivery==='generated'&&isPatternDelivery(profile.delivery)){const beacon=v3(hit.point.x,0,hit.point.z);targeting3d.beacon=beacon;targeting3d.designation=null;const pattern=generateBarragePattern(profile,beacon,THREE.MathUtils.degToRad(+$('targeting3dHeading').value||0),Math.max(1,+$('targeting3dSeed').value||1),{moreGuns:$('targeting3dMoreGuns').checked,atmosphericMonitoring:$('targeting3dAtmospheric').checked,highDensityExplosives:$('targeting3dHighDensity').checked});targeting3d.impacts=pattern.map(resolveVerticalBarrageImpact);}else{const impact=normalizeImpactForDelivery({position:{...hit.point},directPoolKey:hit.directPoolKey,angleIndex:hit.angleIndex||0,time:targeting3d.impacts.length,seed:Math.max(1,+$('targeting3dSeed').value||1)},profile,targeting3d.groundY);targeting3d.beacon=null;targeting3d.designation=null;targeting3d.impacts=delivery==='manual'?[...targeting3d.impacts,impact]:[impact];}runTargeting3dSimulation();$('targeting3dStatus').textContent=profile.delivery.kind==='guided-top-attack'?`Solo Silo top attack resolved at ${fmt(profile.delivery.terminalElevation)}° modeled descent`:profile.delivery.kind==='ground-timed'?`Portable Hellbomb placed on the ground · detonation in ${fmt(profile.delay)} s`:profile.delivery.kind==='eagle-airstrike'?`Six-bomb Eagle Airstrike pattern simulated`:profile.delivery.kind==='eagle-rocket-pods'?`Three two-rocket volleys simulated`:profile.delivery.kind==='eagle-strafing'?`100-round forward Strafing Run simulated`:profile.delivery.kind==='gatling-barrage'?`${targeting3d.impacts.length}-round Orbital Gatling Barrage simulated`:`${targeting3d.impacts.length} impact${targeting3d.impacts.length===1?'':'s'} simulated`;
}

function updateTargeting3dWeapon(){
  targeting3d.profile=selectedTargeting3dProfile();const kind=targeting3d.profile?.delivery.kind||'',patterned=isPatternDelivery(targeting3d.profile?.delivery),barrage=/barrage/.test(kind),gatling=kind==='gatling-barrage',eagle=kind.startsWith('eagle-'),guided=kind==='guided-top-attack',groundTimed=kind==='ground-timed',delivery=$('targeting3dDelivery'),current=delivery.value;[...delivery.options].forEach(option=>option.hidden=!patterned&&option.value!=='single');delivery.value=patterned?(current==='single'?'generated':current):'single';delivery.querySelector('[value="generated"]').textContent=eagle?'Generated Eagle pattern':'Generated barrage';$('targeting3dSeedField').hidden=!barrage;$('targeting3dHeadingField').hidden=!(kind==='walking-barrage'||kind==='eagle-airstrike'||kind==='eagle-rocket-pods'||kind==='eagle-strafing'||guided);$('targeting3dHeadingLabel').textContent=guided?'Approach heading':eagle?'Eagle approach heading':'Walking direction';const note=$('targeting3dGuidanceNote');note.hidden=!guided&&!groundTimed&&!eagle&&!gatling;note.textContent=guided?'Game-derived continuous laser guidance · 15 s motor life · 200 m/s preferred speed. The viewer uses a representative 70° terminal descent; the live angle varies with silo position and laser guidance.':groundTimed?`Dropped backpack device · ground placement only · detonates ${fmt(targeting3d.profile.delay)} seconds after activation.`:kind==='eagle-airstrike'?'Decoded six-bomb 6Z delivery, perpendicular to the throw direction, at 0.2 s intervals. Exact 6Z coordinates are unavailable; the viewer uses a clearly ordered 8 m-spacing representation.':kind==='eagle-rocket-pods'?'Decoded three two-rocket volleys at 0.5 s intervals, guided toward the largest target within 20 m. The tight paired impact layout is a viewer representation of the unrecovered 6Z coordinates.':kind==='eagle-strafing'?'100 rounds at 4000 rpm, traveling forward from the beacon; every fourth round is high-explosive. Exact in-game dispersion is unrecovered, so the viewer uses a seeded 50 m forward lane.':kind==='eagle-500kg'?'Projectile impact plus the decoded 0.8 s delayed main detonation.':gatling?'Four 60-round salvos at 0.045 s intervals; every fourth round is high-explosive. The decoded 7 m area parameter and observed roughly 20 m radius differ, so the viewer uses a labeled 20 m seeded envelope.':'';$('targeting3dUpgrades').hidden=!barrage;$('targeting3dAtmosphericLabel').hidden=!barrage||targeting3d.profile?.delivery.atmosphericEligible===false;$('estimateTargeting3dBarrage').hidden=!barrage||delivery.value==='manual';targeting3d.impacts=[];targeting3d.beacon=null;targeting3d.designation=null;targeting3d.results=null;if(targeting3d.initialized)createTargeting3dGrid();runTargeting3dSimulation();
}
...
function regenerateTargeting3dBarrage(){
  createTargeting3dGrid();if(targeting3d.profile?.delivery.kind==='guided-top-attack'&&targeting3d.designation)placeTargeting3dImpact(targeting3d.designation);else if(targeting3d.beacon&&$('targeting3dDelivery').value==='generated'&&isPatternDelivery(targeting3d.profile?.delivery))placeTargeting3dImpact({point:{...targeting3d.beacon},directPoolKey:null});else runTargeting3dSimulation();
}
...
async function attachTargeting3dMounts(entry,GLTFLoader,baseScene,status,materials){
  ...
  for(const mount of manifest.mounts){
    ...
    if(mount.hitboxAsset&&mount.collisionManifest&&mount.damageManifest){
      ...
      const hitboxAsset=mount.runtimeHitboxAsset||mount.hitboxAsset,hitboxHash=mount.runtimeHitboxAssetSha256||mount.hitboxAssetSha256,hitboxVersion=hitboxHash?`?v=${hitboxHash.slice(0,16)}`:'',[hitboxGltf,collisionManifest,damageManifest]=await Promise.all([
        loadTargeting3dGlb(GLTFLoader,`assets/models/${hitboxAsset}${hitboxVersion}`),
...
async function initializeTargeting3d(enemyName=selectedEnemy().name){
  ...
  try{
    ...
    const collisionBase=`assets/models/${entry.assetSlug||entry.slug}`,damageBase=`assets/models/${entry.damageSlug||entry.slug}`,assetVersion=entry.assetVersion?`?v=${encodeURIComponent(entry.assetVersion)}`:'',renderVersion=entry.renderVersion?`?v=${encodeURIComponent(entry.renderVersion)}`:'',runtimeCollision=entry.collisionGlb||(entry.renderGlb?`${collisionBase}-collision-runtime.glb`:null),paths={glb:runtimeCollision?`${runtimeCollision}${assetVersion}`:`${collisionBase}-collision-research.glb${assetVersion}`,collision:`${collisionBase}-collision-research.manifest.json`,damage:`${damageBase}-damage-zones.manifest.json`};status.textContent=`Loading ${enemyName} model`;
